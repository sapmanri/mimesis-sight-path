...
**두 층 로그 (Vase 판정 2026-07-17) — 하나의 레코드, 두 가지 읽기**

같은 발행 이벤트를 두 관점으로 본다. **저장은 한 레코드**, 표현만 분리한다.

**Layer 1 — 운영 로그 (개발자용)**: 장애 진단. `runId · scheduledFor · httpStatus · result · threads.errorCode · requestId`

**Layer 2 — 별이 일지 (우리가 보는 화면)**: 별이의 행동으로 서술.
```
22:00
  오늘의 42번째 문장을 골랐다.        ← textIndex (문장 원문은 feed에서)
  걷는 사진 한 장을 함께 걸었다.       ← imageKey 있음 / 없으면 "사진 없이"
  Threads에 올렸다. ✓                 ← result
```
> ⚠️ **아는 것만 말한다.** 사진의 피사체("벤치")는 **메타가 없어 알 수 없다**(§4). 자동 캡처 파이프라인이
> 존·행동 메타를 함께 저장하기 전까지 별이 일지는 **"걷는 사진 한 장"** 까지만. `record`를 촬영으로
> 단정하지 않은 것과 같은 규율. 파이프라인 도입 후 "벤치를 지나며 찍은 사진"으로 승격.

레코드에 두 층 모두의 소스를 담는다(추가 필드 없음 — 기존 값 재배치):
`{runId, scheduledFor, invokedAt, result, httpStatus, textIndex, imageKey, threads:{...}}`
→ Layer 1은 `httpStatus/errorCode/requestId`를, Layer 2는 `textIndex/imageKey/result`를 읽는다.
...
콘솔 v1 표현 (Vase 판정 — "미구현" 한 마디보다 3층으로 나눠 운영자가 즉시 이해):
```
운석우
  상태     준비중
  조건     충족 (night · clear)      ← state.sky로 실시간 계산 ✅
  발동     미구현 (런타임 없음)       ← 423-EVENTS 대기
  발생기록  없음
```
- **상태 "준비중"**: 정의는 살아있고 조건 계산도 되는데 발사 장치만 없음을 한 단어로
- **조건**: `state.sky.phase/weather` vs 레지스트리 `eligibleTime/eligibleWeather` 대조 → 충족/미충족 실시간
- **발동 "미구현"**: 운영자가 "조건은 맞는데 런타임이 없구나"를 바로 이해 → 정확히 §5-2 423-EVENTS를 가리킴
...
### 9-0. 🌟 "오늘의 별이" 상시 카드 (Vase 판정 — 오른쪽 위 고정)

숫자 나열보다 **"오늘 별이는 어떤 하루를 보내고 있는가"** 가 먼저 보여야 한다.
운영 콘솔이지만 우리에겐 **별이의 하루를 들여다보는 창**이다. 오른쪽 최상단, 스크롤해도 고정.

```
오늘의 별이
──────────────
걷는 중 · 2시간 13분        ← state.byeoli.state + 가동 후 경과
사진 4장                    ← 오늘 feed[].img 개수 (별이 누적 아님)
기억 18개 · 취향 2개        ← ⚠ 아래 주의
Threads 다음 예정 · 22:00   ← publish_log missed/다음 슬롯
```

> ⚠️ **데이터 정직성 (착수 전 확정 필요)**:
> - "걷는 중 N시간" → `state.byeoli.state` + `health.startedAt` 경과. 단 startedAt은 **Authority 인스턴스 시작**이라 "오늘"과 다를 수 있음 → **"이번 세션 N시간"** 이 정확
> - "사진 4장" → 오늘 `feed[].img` 개수로 산출 가능 ✅
> - "기억 18개 · 취향 2개" → 🔴 **"오늘 별이의 기억 증가분"은 현재 못 구한다.** `telemetry.memories`는 **누적**이고 일별 스냅샷이 없음. 이걸 구하려면 콘솔이 자정 스냅샷을 저장해야 함(별도). **v1은 누적값 + "누적"이라고 명시**하거나 이 줄을 빼고 착수. 422-OPS-C에서 확정.
