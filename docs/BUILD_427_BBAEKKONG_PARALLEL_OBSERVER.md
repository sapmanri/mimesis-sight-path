...
## 8. 최종 QC (Vase 2026-07-19) — 승인 · 3건 추가 반영

| QC | 판정 | 반영 |
|---|---|---|
| 1 | **eventId 절대 유지** — 향후 Ops에서 같은 대상의 두 기록을 묶어 보는 열쇠 | 계약으로 고정 (제거 금지) |
| 2 | 모바일 "숨김≠없음" 유지 — 생성O·저장O·표시X. 후일 "빼콩 보기" 버튼이면 전부 살아남 | 유지 |
| 3 | 색만으론 부족 → **이름표** | `.who` — 모든 빼콩 줄에 `🐾 빼콩` 라벨 (캐릭터 확장 대비) |
| 4 | **"둘이 같은 걸 봤다" 표시** | eventId 있는 줄에 🔗 아이콘(+툴팁) |
| 5 | **말수 비율 별이 100 : 빼콩 30~40** — 주인공은 별이 | `voiceRatioMax:0.4` — 화면의 별이 발화 대비 상한, 별이가 조용하면 빼콩도 조용. return(상태 근거 사건)만 예외 |

**후속 (지금 구현 안 함 — 방향 원칙으로 고정)**
- 빼콩이 가끔 **별이를 바라보는 일기**: "별이가 꽃 앞에서 한참 서 있었다." — 실제 상태(byeoli.actTarget·지속시간) 근거로만
- **역방향 절대 금지**: 별이는 빼콩을 기록하지 않는다. 별이는 세상을 보는 아이, 빼콩은 별이를 포함한 세상을 보는 아이 — 이 비대칭이 캐릭터의 핵
- "발이 젖어 있다"처럼 **상태에서 문장이 태어나는 구조**가 게놈·Writing Studio 도입 후에도 이어질 출발점 (Vase 판정문)
