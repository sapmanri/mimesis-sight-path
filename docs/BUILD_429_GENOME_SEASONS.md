# BUILD 429 — Genome Seasons · **Observation Compiler Complete** (정본)

> **Genome은 문체를 저장하지 않는다. 무엇을 선택해, 어떻게 바라보는지를 저장한다. 문체는 그 결과다.**
>
> **최종 판정 (Vase 2026-07-19)**: 429부터 이 프로젝트는 문장을 만드는 프로젝트가 아니라
> **관찰을 컴파일하고, 국소 수정하고, 실패하면 롤백하는 시스템**이다.
>
> | 층 | 판정 |
> |---|---|
> | Generation Engine | ✅ PASS — Contract 100% · semantic 0 · core 0 · reuse 0 · fallback 0. 생성기는 더 이상 문제가 아니다 |
> | Repair Engine | ✅ PASS — 문제 2개면 2개만 교체 → 원자적 저장 → 실패 시 원본 보존. 이번 빌드 최대 성과 |
> | Observation Memory | ✅ PASS — semanticSignature 실효 증명 (표면 0 · core 0 · semantic 0) |
> | Critic | ❌ FAIL (좋은 FAIL) — "틀린 문장을 잡는다"가 아니라 "매번 다른 문장을 잡는다". 문제는 이제 Generator가 아니라 **Judge**다 |
>
...
**수렴 규칙 확정 (Vase 2026-07-19): ③ 기계 규칙 승격.**
critic이 자연어로 이유를 말하게 하지 말고, 판정을 **Rule 코드로 귀결**시킨다:

```
FAIL → category_alignment → reason: non-exclusive target
```

같은 Rule이면 다음 실행에서도 같은 결과 — **Critic도 AI가 아니라 Execution Contract가 되어야 한다.**
- ① "한번 발견한 위반 목록만 끝까지 수리"는 **반대** — critic이 틀렸을 수 있다.
- ② 2-pass 합의는 가능하나, critic에 랜덤성이 있으면 A→B→A→C로 계속 흔들릴 수 있다.

**다음 빌드의 목표는 하나다: Generator 개선이 아니라 Critic의 계약화.**
(빌드 번호는 Vase 확정 대기 — 430은 Parallel Lives로 이미 사용됨.)
