...
> **역사 문서 — 2026-08-14 이후 운영 정본 아님.** 이 문서의 30% 상한·숙성·사람 Draft/승인
> 설계와 08/18/22 고정 발행은 폐기되었다. 현재 정본은
> `docs/BYEOLI_THREADS_SOCIAL_DIRECTOR_20260814.md`다. 아래 내용은 당시 판단과 사고 경위를
> 보존하기 위해 수정하지 않는다.

작성: 2026-07-17 밤 · 상태: **Vase 승인(같은 날) — 425-A 착수.** 판정: ⭐ 기억해둠만(❤️ 보류) · 답글 상한 = **일 개수제 폐기 → 해당일 전체 신규 댓글의 ~30%**(여타 쿨다운 규칙은 유지) · 승인 UI = 콘솔 내 섹션(권고안 채택)
...
### 4-3. 답글 정책 (Vase 판정 2026-07-17로 개정)
**상한: 해당일 전체 신규 댓글의 ~30% (반올림)** — 개수제(일 3~5건) 폐기. 댓글이 많아지면 답글도 자연스럽게 늘고, 적으면 조용해진다.
유지되는 규칙: 댓글 10분 숙성 후 후보 · 게시물당 1~2건 · 같은 계정 24h 1회 · 이모지/ㅋㅋ/스팸 무응답 · 민감 주제(비난·정치·의료·법률·개인정보) 자동 발행 금지 · 애매하면 무응답 · 발행은 즉시 아닌 지연. (30%는 상한이지 목표가 아니다 — 쿨다운 규칙들이 우선한다)
...
**판정 완료(2026-07-17)**: ① 설계 승인 ✅ ② ⭐ 기억해둠만, ❤️ 보류 ✅ ③ 승인 UI = 콘솔 섹션 ✅ ④ 토큰 재발급 — Vase 대기. → **425-A(엽서) 착수.**
