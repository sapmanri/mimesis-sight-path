...
export function MemoryRoadSkin({ scenes, activeIndex }: RoadSkinProps) {
  ...
  return (
    <group>
      {pieces.map((piece) => {
        const focus = Math.max(0.18, 1 - Math.abs(piece.nearestIndex - activeIndex) * 0.1);
        ...
        if (piece.kind === 'wood') {
          return (
            <group key={piece.id} position={piece.position} rotation={piece.rotation} scale={piece.scale}>
              <mesh scale={[1, 0.14, 0.18]}>
                <boxGeometry args={[1, 1, 1]} />
                <meshStandardMaterial color={piece.color} roughness={1} transparent opacity={opacity} />
              </mesh>
              <mesh position={[0.22, 0.03, 0.02]} rotation={[0.1, 0.25, -0.08]} scale={[0.46, 0.08, 0.12]}>
                <boxGeometry args={[1, 1, 1]} />
                <meshStandardMaterial color="#6e5c49" roughness={1} transparent opacity={opacity * 0.72} />
              </mesh>
            </group>
          );
        }
...
function buildRoadSkin(scenes: ObservationScene[]): ErosionPiece[] {
  ...
  const density = activeMemoryBiome.density;
  ...
  scenes.slice(0, -1).forEach((scene, index) => {
    ...
    const count = Math.max(5, Math.round(8 + density * 8));

    for (let i = 0; i < count; i += 1) {
      const t = (i + random() * 0.7) / count;
      const center = start.clone().lerp(end, t);
      const side = random() > 0.5 ? 1 : -1;
      const edgeDistance = 0.54 + random() * 0.5;
      const alongJitter = (random() - 0.5) * 0.36;
      const point = center
        .clone()
      ...
      const roll = random();
      const kind: ErosionPiece['kind'] = roll > 0.84
        ? 'wood'
        : roll > 0.64
          ? 'edge-collapse'
          : roll > 0.38
      ...
      const y = kind === 'edge-collapse' ? -0.82 - random() * 0.26 : -0.515 + random() * 0.03;
      ...
      pieces.push({
        ...
        rotation: kind === 'dust' || kind === 'crack'
          ? [-Math.PI / 2, 0, angle + (random() - 0.5) * 1.1]
          : [random() * 0.6, angle + random() * 0.7, (random() - 0.5) * 0.7],
...
function pickColor(kind: ErosionPiece['kind'], palette: typeof activeMemoryBiome.palette, r: number) {
  if (kind === 'wood') return r > 0.5 ? '#8b7258' : '#6e5c49';
  ...
  return r > 0.5 ? '#9b9279' : palette.structure;
}

function scaleForKind(kind: ErosionPiece['kind'], random: () => number): [number, number, number] {
  if (kind === 'wood') return [0.22 + random() * 0.32, 0.32 + random() * 0.3, 0.32 + random() * 0.3];
  if (kind === 'crack') return [0.22 + random() * 0.48, 0.018, 0.018];
  if (kind === 'dust') return [0.08 + random() * 0.22, 0.04 + random() * 0.1, 1];
  if (kind === 'edge-collapse') return [0.12 + random() * 0.24, 0.08 + random() * 0.18, 0.14 + random() * 0.28];
  return [0.08 + random() * 0.18, 0.04 + random() * 0.12, 0.08 + random() * 0.18];
}

function opacityForKind(kind: ErosionPiece['kind'], density: number, r: number) {
  if (kind === 'crack') return 0.16 + density * 0.12 + r * 0.08;
  if (kind === 'dust') return 0.08 + density * 0.08 + r * 0.06;
  if (kind === 'edge-collapse') return 0.28 + density * 0.16 + r * 0.12;
  if (kind === 'wood') return 0.24 + density * 0.18 + r * 0.1;
  return 0.22 + density * 0.14 + r * 0.12;
}
