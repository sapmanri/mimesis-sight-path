...
export function buildInvestigatePrompt(intent: SearchIntent, queries: QueryPlan[]): string {
  return [
    ...
    `  2. 그중 실제로 읽을 값이 있는 글을 web_fetch로 **펼쳐 읽는다.**`,
    `     · 리뷰·인터뷰·에세이를 우선해라. 누군가 그 음악을 **듣고 쓴 글**이어야 한다.`,
    `     · 백과사전 항목(나무위키·위키백과)은 이름·연도를 확인하는 데는 쓰되,`,
    `       그것만 읽고 고르지 마라. 사전은 그 곡이 어떤지 말해주지 않는다.`,
    `     · **서로 다른 사이트를 최소 두 곳 이상 읽어라.** 한 곳만 깊게 파면 그 사이트의`,
    `       취향을 네 취향으로 착각하게 된다. 한 글만 읽고 여러 곡을 고르는 것도 안 된다 —`,
    `       그건 찾은 게 아니라 그 글의 목록을 베낀 것이다.`,
    `  3. 읽은 내용으로 곡을 고른다.`,
    ``,
    `⚠ 규칙 — 어기면 그 곡은 버려진다:`,
    ...
    `  · byeoliSummary는 **네 해석**이고, 사람이 실제로 읽게 되는 문장이다. 두세 문장으로.`,
    `    그 음악을 어떻게 들었는지만 써라. 검색·읽기·피하기 같은 **네가 일한 과정은 쓰지 마라** —`,
    `    읽는 사람은 그런 단계가 있다는 걸 모른다. because에만 적어라.`,
    `  · 가사나 리뷰 원문을 옮기지 마라.`,
    `  · ⚠ **앨범이 아니라 곡 하나를 골라라.** 재생목록에는 앨범을 담을 수 없다.`,
    `    읽은 글이 앨범을 다루면 그 앨범에서 곡 하나를 정해서 그 곡 이름을 써라.`,
    `    앨범 이름을 title에 적으면 서가에서 찾지 못하고 그날 목록에서 빠진다.`,
    `  · 중심곡(role: center)은 하루에 하나뿐이다. 나머지는 around.`,
    `  · **같은 곡을 두 번 고르지 마라.** 같은 곡의 다른 판본(다른 연주자·다른 해)도 안 된다.`,
    `    하루의 묶음은 서로 다른 곡이어야 한다.`,
