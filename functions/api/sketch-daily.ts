...
//     ③ 물린 이유가 다음 장 프롬프트에 실린다 — 같은 이유로 셋이 물리던 21일의 정체를 친다.
  ...
  // ⚖ 09-02 사장 판정: **「사물만 있는 날이든 뭐든 캐릭터는 둘 다 항상 등장한다. 이게 있어야 해.」**
  //   08-30에 「별이가 선택하게 해」를 *참조 선택*으로 받아 별이가 상대를 고르게 만들었는데,
  //   그게 이 규칙을 깼다. 실제로 09-01 밤 별이는 세 번 다 「오늘 기억엔 우산·창·바구니뿐」이라며
  //   아무도 안 불렀고, 참조 없이 그려진 그림은 별이도 사장도 「별이가 아니다」로 물렸다.
  //   8-30 문제(그날 주인공이 개인데 고양이가 그려짐)의 해법은 캐릭터를 빼는 게 아니라
  //   **그날의 주제를 가장 크게 세우는 것**이었다 — 그건 프롬프트가 한다(_daily-sketch).
  //   그래서 선택 단계를 걷어낸다. 캐릭터 참조는 화면이 배정한 것을 늘 전부 쓴다.
