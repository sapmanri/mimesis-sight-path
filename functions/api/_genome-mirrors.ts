...
export function buildScenarioSystemV2(castIds: string[]):
  ...
  lines.push(
    ...
    '',
    '## 카메라 리듬 (Camera Bible v0 — 홈즈 설계 07-23: "평평했던 이유는 내용이 아니라 카메라였다")',
    '- 컷마다 거리(framing)가 변해야 한다 — medium 연속 금지. wide→medium→close→wide 같은 리듬으로.',
    '- 발견은 가까워진다: discovery 순간은 close — 얼굴·손·파형을 크게.',
    '- 생각은 비운다: 생각하는 컷은 공간과 여백을 크게 — 파형이 공간을 차지하게 (setting에 여백 명시).',
    '- 웃음은 움직인다: 정면보다 사선, 몸이 풀리고 파형이 흔들리는 action으로.',
    '- 착지는 멀어진다: 마지막 컷은 wide — 방 전체, 창문, 혼자.',
    '',
    '',
    ...
    '- outro: 관찰 일지의 마지막 메모 — 사건을 정리하지 말고 발견 하나를 적는다. (예: "투고는 사라졌지만, 그 대화는 기록으로 남았다.")',
